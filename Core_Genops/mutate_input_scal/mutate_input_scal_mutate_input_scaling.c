...
bool matchmutate_input_scal_mutate_input_scaling(Morphism *morphism)
{
   if(1 > mutate_input_scal_host->number_of_nodes || 0 > mutate_input_scal_host->number_of_edges) return false;
   if(match_n0(morphism)) return true;
   else
   {
      initialiseMorphism(morphism, mutate_input_scal_host);
      return false;
   }
}
...
{
   int host_index;
   for(host_index = 0; host_index < mutate_input_scal_host->nodes.size; host_index++)
   {
      Node *host_node = getNode(mutate_input_scal_host, host_index);
      ...
      do
      {
         ...
         else if(strcmp(item->atom.str, "INPUT SCALING") != 0) break;
         item = item->next;
...
void applymutate_input_scal_mutate_input_scaling(Morphism *morphism, bool record_changes)
{
   int host_node_index = lookupNode(morphism, 0);
   HostLabel label_n0 = getNodeLabel(mutate_input_scal_host, host_node_index);
   HostLabel label;
   ...
   array0[index0++].str = "INPUT SCALING";
   ...
   else
   {
      ...
      relabelNode(mutate_input_scal_host, host_node_index, label);
   }
   /* Reset the morphism. */
   initialiseMorphism(morphism, mutate_input_scal_host);
}
...
bool fillpotmutate_input_scal_mutate_input_scaling(MorphismPot *pot, Morphism *morphism)
{
   if(1 > mutate_input_scal_host->number_of_nodes || 0 > mutate_input_scal_host->number_of_edges) return false;
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < mutate_input_scal_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "mutate_input_scal_mutate_input_scaling", mutate_input_scal_host);
