...
{
   if(2 > add_weight_input_host->number_of_nodes || 1 > add_weight_input_host->number_of_edges) return false;
...
static bool match_e0(Morphism *morphism)
{
   ...
   Node *host_node = getNode(add_weight_input_host, start_index);

   int counter;
   for(counter = 0; counter < host_node->out_edges.size + 2; counter++)
   {
      Edge *host_edge = getNthOutEdge(add_weight_input_host, host_node, counter);
      ...
      {
         Node *end_node = getNode(add_weight_input_host, host_edge->target);
...
static bool match_n1(Morphism *morphism, Edge *host_edge)
{
   Node *host_node = getTarget(add_weight_input_host, host_edge);
...
void applyadd_weight_input_removeEdge(Morphism *morphism, bool record_changes)
{
   ...
   if(record_changes)
   {
      Edge *edge = getEdge(add_weight_input_host, host_edge_index);
      ...
      pushRemovedEdge(edge->label, edge->source, edge->target, edge->index,
                      edge->index < add_weight_input_host->edges.size - 1);
   }
   removeEdge(add_weight_input_host, host_edge_index);
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism, Edge *host_edge)
{
   ...
   if(match)
   {
      ...
      host_node->matched = true;
      /* All items matched! */
         putMorphism(pot, morphism, "add_weight_input_removeEdge", add_weight_input_host);
