...
bool matchmutate_spec_rad_mutate_spectral_radius(Morphism *morphism)
{
   if(1 > mutate_spec_rad_host->number_of_nodes || 0 > mutate_spec_rad_host->number_of_edges) return false;
   if(match_n0(morphism)) return true;
   else
   {
      initialiseMorphism(morphism, mutate_spec_rad_host);
      return false;
   }
}
...
{
   int host_index;
   for(host_index = 0; host_index < mutate_spec_rad_host->nodes.size; host_index++)
   {
      Node *host_node = getNode(mutate_spec_rad_host, host_index);
      ...
      do
      {
         ...
         else if(strcmp(item->atom.str, "SPECTRAL RADIUS") != 0) break;
         item = item->next;
...
void applymutate_spec_rad_mutate_spectral_radius(Morphism *morphism, bool record_changes)
{
   int host_node_index = lookupNode(morphism, 0);
   HostLabel label_n0 = getNodeLabel(mutate_spec_rad_host, host_node_index);
   HostLabel label;
   ...
   array0[index0++].str = "SPECTRAL RADIUS";
   ...
   else
   {
      ...
      relabelNode(mutate_spec_rad_host, host_node_index, label);
   }
   /* Reset the morphism. */
   initialiseMorphism(morphism, mutate_spec_rad_host);
}
...
bool fillpotmutate_spec_rad_mutate_spectral_radius(MorphismPot *pot, Morphism *morphism)
{
   if(1 > mutate_spec_rad_host->number_of_nodes || 0 > mutate_spec_rad_host->number_of_edges) return false;
...
static bool fillpot_n0(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(host_index = 0; host_index < mutate_spec_rad_host->nodes.size; host_index++)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "mutate_spec_rad_mutate_spectral_radius", mutate_spec_rad_host);
