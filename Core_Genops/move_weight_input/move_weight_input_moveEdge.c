...
{
   if(3 > move_weight_input_host->number_of_nodes || 1 > move_weight_input_host->number_of_edges) return false;
...
{
   RootNodes *nodes;
   for(nodes = getRootNodeList(move_weight_input_host); nodes != NULL; nodes = nodes->next)
   {
      Node *host_node = getNode(move_weight_input_host, nodes->index);
...
static bool match_e0(Morphism *morphism)
{
   ...
   Node *host_node = getNode(move_weight_input_host, start_index);

   int counter;
   for(counter = 0; counter < host_node->out_edges.size + 2; counter++)
   {
      Edge *host_edge = getNthOutEdge(move_weight_input_host, host_node, counter);
      ...
      {
         Node *end_node = getNode(move_weight_input_host, host_edge->target);
...
static bool match_n1(Morphism *morphism, Edge *host_edge)
{
   Node *host_node = getTarget(move_weight_input_host, host_edge);
...
void applymove_weight_input_moveEdge(Morphism *morphism, bool record_changes)
{
   ...
   if(record_changes)
   {
      Edge *edge = getEdge(move_weight_input_host, host_edge_index);
      ...
      pushRemovedEdge(edge->label, edge->source, edge->target, edge->index,
                      edge->index < move_weight_input_host->edges.size - 1);
   }
   removeEdge(move_weight_input_host, host_edge_index);
   ...
   changeNodeMark(move_weight_input_host, host_node_index, 0);
   ...
   HostLabel label_n1 = getNodeLabel(move_weight_input_host, host_node_index);
   ...
   HostLabel label_n2 = getNodeLabel(move_weight_input_host, host_node_index);
   ...
   changeRoot(move_weight_input_host, host_node_index);
   int source, target;
   int edge_array_size0 = move_weight_input_host->edges.size;
   ...
   host_edge_index = addEdge(move_weight_input_host, label, source, target);
   /* If the edge array size has not increased after the edge addition, then
      the edge was added to a hole in the array. */
   if(record_changes)
      pushAddedEdge(host_edge_index, edge_array_size0 == move_weight_input_host->edges.size);
   /* Reset the morphism. */
   initialiseMorphism(morphism, move_weight_input_host);
}
...
static bool fillpot_n2(MorphismPot *pot, Morphism *morphism)
{
   ...
   for(nodes = getRootNodeList(move_weight_input_host); nodes != NULL; nodes = nodes->next)
   {
      ...
      if(match)
      {
         ...
         host_node->matched = true;
         /* All items matched! */
            putMorphism(pot, morphism, "move_weight_input_moveEdge", move_weight_input_host);
