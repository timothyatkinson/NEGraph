...
{
   if(2 > move_weight_input_host->number_of_nodes || 1 > move_weight_input_host->number_of_edges) return false;
...
void applymove_weight_input_pickWeight(Morphism *morphism, bool record_changes)
{
   ...
   HostLabel label_e0 = getEdgeLabel(move_weight_input_host, host_edge_index);
   if(record_changes) pushRemarkedEdge(host_edge_index, label_e0.mark);
   changeEdgeMark(move_weight_input_host, host_edge_index, 2);
   ...
   changeNodeMark(move_weight_input_host, host_node_index, 2);
   ...
   host_node_index = lookupNode(morphism, 1);
   Node *node1 = getNode(move_weight_input_host, host_node_index);
   if(!node1->root)
   {
      if(record_changes) pushChangedRootNode(host_node_index);
      changeRoot(move_weight_input_host, host_node_index);
...
static bool fillpot_n1(MorphismPot *pot, Morphism *morphism, Edge *host_edge)
{
   ...
   if(match)
   {
      ...
      host_node->matched = true;
      /* All items matched! */
         putMorphism(pot, morphism, "move_weight_input_pickWeight", move_weight_input_host);
